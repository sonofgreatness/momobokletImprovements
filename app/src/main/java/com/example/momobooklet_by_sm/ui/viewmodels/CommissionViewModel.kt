...
import com.example.momobooklet_by_sm.services.csv.adapters.DailyCommissionModelCSV
import com.example.momobooklet_by_sm.services.csv.adapters.TransactionModelCSV
import com.example.momobooklet_by_sm.services.csv.adapters.dailyCommissiontoCsv
...
class CommissionViewModel   (val application: Application,
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    val datesManager: CommisionDatesManager = CommisionDatesManager(activity)
    ...
    fun setCustomDaysCommissionModel(date: String) {
        viewModelScope.launch {
            commission_repository.getDaysCommission(date).collect {
    ...
    fun writeReport(startDate:String,type:ReportType)
    {
        val dates: List<String> = dateMaker(type, startDate)
          ...
          if (type.ordinal > 3)
              writeCSVReport(dates, CsvConfig(application,type.name,datesManager,WRITETO.EXTERNAL))

          else
                  writePDFReportTransactions(
                      dates,
                      PdfConfig(application, type.name, datesManager, WRITETO.EXTERNAL))
    }




    private fun writeCSVReport(dates:List<String>,csvConfig: CsvConfig) {
     ...
     ***********************************************************************/
    private fun exportTransactionsAndCommissionsToCsv(dates: List<String>, csvConfig: CsvConfig) =
     ...
     ***********************************************************/
   private fun writePDFReportTransactions(dates: List<String>,pdfConfig:PdfConfig)
    {
        exportTransactionsAndCommissionsToPdf(dates, pdfConfig)
    }

    private fun exportTransactionsAndCommissionsToPdf(dates: List<String>, pdfConfig: PdfConfig) = viewModelScope.launch(IO) {
        // 👇 state manager for loading | error | success
        // _exportCsvState.value = ViewState.LoadingSuccess
